...
// Activates the random color updates at each frame
//#define DYN_COL_UPDATE		    

// Uses the glMap, so that we can directly update the data by mapping the data into the CPU space
//#define MAP_POLICY			

/**
 *  This macro defines wich is the value of one single element
 */
#define GL_FLOAT_SIZE			(sizeof(Vector4)/4)

 /**
  *   This function performs the memory mapping, either with the custom method, or with the Map/Unmap one.
  *   @param id:		pointer to the data structure containing the reference to the element sent to the buffer
  *   @param data:	raw data contained in the CPU program
  *   @param dim:	vector dimension
  *   @param n:		number of vectors
  *   @param type: type of the array
  *   @param enumType	    enumType associated to the object
  *
  */
bool mapBuffer(unsigned int* id, void* data, unsigned int dim, unsigned int n, unsigned int type, unsigned int enumType, const unsigned int sizeOfOneDataType) {
	if (!id)
		return false;
	unsigned int sizeOne = dim * sizeOfOneDataType;
	unsigned int size = sizeOne * n;

	// If the element has not been initialized, then I need to allocate a fresh name
	bool doInit = true;//!(*id);
	if (doInit) {
		glGenBuffers(1, id);
	}
...
#ifdef MAP_POLICY
	// If I need to update, then use the glMapBuffer method to update the memory within the CPU space
	if (!doInit) {
		...
		// Else, initializing the memory as before
		glBindBuffer(type, *id);
		glBufferData(type, size, data, GL_STATIC_DRAW);

		if (enumType != UINT_MAX) {
			glVertexAttribPointer(enumType, dim, GL_FLOAT, GL_FALSE, 0, 0);
			glEnableVertexAttribArray(enumType);
		}
	}
#else
	glBindBuffer(type, *id);
	...
	// make sure to tell OpenGL we're done with the pointer. This is required for not allocating memory at each update without freeing it
	glUnmapBuffer(type);
...
void UploadAttribute(GLuint* id, int numElements, int dataSize, int attribSize, int attribID, void* pointer, const string&debugName, bool doUpdate = false) {
...
//GenerateTangents();
	...
	glBindVertexArray(arrayObject);
	mapBuffer(&bufferObject[VERTEX_BUFFER], vertices, 3, numVertices, GL_ARRAY_BUFFER, VERTEX_BUFFER, GL_FLOAT_SIZE);

	//Buffer texture data
	if (textureCoords) {
		mapBuffer(&bufferObject[TEXTURE_BUFFER], textureCoords, 2, numVertices, GL_ARRAY_BUFFER, TEXTURE_BUFFER, GL_FLOAT_SIZE);
	}
	
	//buffer colour data
	if (colours) {
		mapBuffer(&bufferObject[COLOUR_BUFFER], colours, 4, numVertices, GL_ARRAY_BUFFER, COLOUR_BUFFER, GL_FLOAT_SIZE);
	}
	
	//Buffer normal data
	if (normals) {
		mapBuffer(&bufferObject[NORMAL_BUFFER], normals, 3, numVertices, GL_ARRAY_BUFFER, NORMAL_BUFFER, GL_FLOAT_SIZE);
	}

	//Buffer tangent data
	if (tangents) {
		mapBuffer(&bufferObject[TANGENT_BUFFER], tangents, 3, numVertices, GL_ARRAY_BUFFER, TANGENT_BUFFER, GL_FLOAT_SIZE);
	}
	
	//Buffer weights data
	if (weights) {
		mapBuffer(&bufferObject[WEIGHTVALUE_BUFFER], weights, 4, numVertices, GL_ARRAY_BUFFER, WEIGHTVALUE_BUFFER, GL_FLOAT_SIZE);
	}

	//Buffer weight indices data...uses a different function since its integers...
	if (weightIndices) {
		mapBuffer(&bufferObject[WEIGHTINDEX_BUFFER], weightIndices, 4, numVertices, GL_ARRAY_BUFFER, WEIGHTINDEX_BUFFER, sizeof(GLuint));
	}
	...
	*/
	//buffer index data
	if (indices) {
		mapBuffer(&bufferObject[INDEX_BUFFER], indices, 1, numIndices, GL_ELEMENT_ARRAY_BUFFER, UINT_MAX, sizeof(GLuint));
	}
